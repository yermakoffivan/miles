...
The dispatched workflow and its reusable workflows come from the reviewed default-branch commit. A trusted resolver reads the separately checked-out PR snapshot without importing it, maps the requested registration to a fixed CPU/CUDA suite and runner, and the reusable workflow then checks out that same exact SHA to run only the requested file through `run_suite.py --files`. The command supports only same-repository PRs and becomes available after `run-ci-file.yml` exists on the default branch; a later push does not change the already dispatched snapshot.
...
The handler runs only fixed, reviewed code from the default branch and never checks out or executes PR code, dependencies, artifacts, or configuration. The fixed file-run workflow resolves and executes the exact PR snapshot separately, passes only the test secrets required by the selected reusable workflow, and never receives the command gateway App private key. Initial authorization, PR, policy, or run-state errors fail before any mutation. A recheck error before a later rerun request stops that request, while any earlier accepted reruns remain applied.
